...
exports.driverView = async (req) => {
  try {
    ...
    if (req.query.operatorId) {
      const opId = new mongoose.Types.ObjectId(req.query.operatorId);
      conditions.push({
        $match: {
          operator_id: opId,
        },
    ...
    if (req.query.sortByPendingAmount) {
      let sortByPendingAmount =
        req.query.sortByPendingAmount === "desc" ? -1 : 1;
      sortConditions.pending_balance = sortByPendingAmount;
    }

    if (req.query.sortBycurrentStatus) {
      let sortBycurrentStatus =
        req.query.sortBycurrentStatus === "desc" ? -1 : 1;
    ...
    if (req.query.sortByVehicleStatus) {
      let sortByVehicleStatus =
        req.query.sortByVehicleStatus === "desc" ? -1 : 1;
...
exports.driverUpdate = async ({ body, file, params }) => {
  try {
    ...
    const updateData = await driverModel.findByIdAndUpdate(params.id, body, {
      new: true,
    });
    if (!updateData) {
      return {
...
exports.driverDelete = async (req) => {
  try {
    const deleteData = await driverModel.findByIdAndDelete({
      _id: new mongoose.Types.ObjectId(req.params.id),
    });
    if (deleteData) {
      return {
...
exports.editBlockDriver = async (req) => {
  try {
    const getData = await driverModel.findOne(
      { _id: req.params.id },
      { first_name: 1, email: 1, mobile: 1, profile_image: 1, last_name: 1 }
    );
    return {
      statusCode: statusCode.OK,
      success: true,
...
exports.updateDocumentStatus = async (req) => {
  try {
    ...
    if (
      ...
      (!data.providerTaxiDocuments ||
        !data.providerTaxiDocuments[document_type])
    ) {
      return {
        statusCode: statusCode.BAD_REQUEST,
        status: statusCode.BAD_REQUEST,
